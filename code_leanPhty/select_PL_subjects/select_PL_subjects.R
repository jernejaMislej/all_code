...
#2.take bottom n% of ratio z-score and those that are normal/undrweight(less broad)
#2.1 bottom 25% of ratio z-score
#2.2 bottom 30% of ratio z-score
#2.3 bottom 35% of ratio z-score
#3. set cut off point for the environment score only and take those that are normal/underweight(much less broad)-there is no environment score, it is fitted values
# of bmi residuals model, so taking any kind of top n% is taking those that were predicted to have a high bmi, but considering that the residuals are almost identical to bmi
# it self, this is as taking high bmi itself, all this makes sense since R² is so low, means that almost no variation is explained, so the residuals are almost as bmi itself
