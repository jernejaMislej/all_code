...
VIP_data_independant_complete_cases <- na.omit(VIP_data_independant[,c("Subject_id","bmi","bmi_norm_sd","age","agesq","gender_factor","year","ffq_factor",
						...
						"kalcsum1_TEI_adjusted_norm_sd","KALIsum1_TEI_adjusted_norm_sd","enummer","fibesum1_TEI_adjusted_norm_sd")])
...
selected_model<-lm(log(bmi)~age + agesq + gender_factor + year + ffq_factor + POLYsum1_TEI_adjusted_norm_sd+MONOsum1_TEI_adjusted_norm_sd+mfetsum1_TEI_adjusted_norm_sd+
				...
				JODIsum1_TEI_adjusted_norm_sd+kalcsum1_TEI_adjusted_norm_sd+KALIsum1_TEI_adjusted_norm_sd+g6+alkosum1_TEI_adjusted_norm_sd, data = VIP_data_independant)
...
#visit1
VIP_data_subset_visit1_selected<-VIP_data_subset_visit1[,selected_variables]
...
residuals_visit1<-log(VIP_data_subset_visit1$bmi)-predicted_values_visit1$fit

#visit2
VIP_data_subset_visit2_selected<-VIP_data_subset_visit2[,selected_variables]
...
residuals_visit2<-log(VIP_data_subset_visit2$bmi)-predicted_values_visit2$fit



#-------------resistance_continuous-------------------
VIP_data_subset_visit1$resistance_continuous<-residuals_visit1
#-------------resistance_continuous-------------------


#-------------resistance_continuous-------------------
VIP_data_subset_visit2$resistance_continuous<-residuals_visit2
#-------------resistance_continuous-------------------
...
subjects_total1<-VIP_data_subset_visit1$Subject_id[VIP_data_subset_visit1$enummer %in% link_data$enummer]

subjects_total2<-VIP_data_subset_visit2$Subject_id[VIP_data_subset_visit2$enummer %in% link_data$enummer]

subject_total<-intersect(subjects_total1,subjects_total2)

#get the enummers for those subjects(from the first set)
enummers_total<-VIP_data_subset_visit1$enummer[VIP_data_subset_visit1$Subject_id %in% subject_total]
...
final_dataset$resistance_phenotype<-as.numeric((VIP_data_subset_visit1$resistance_continuous[VIP_data_subset_visit1$Subject_id %in% subject_total]+
			VIP_data_subset_visit2$resistance_continuous[VIP_data_subset_visit2$Subject_id %in% subject_total])/2)

final_dataset$mean_bmi<-(VIP_data_subset_visit1$bmi[VIP_data_subset_visit1$Subject_id %in% subject_total]+
		VIP_data_subset_visit2$bmi[VIP_data_subset_visit2$Subject_id %in% subject_total])/2
...
write.table(final_dataset, "../Results/persistantly_lean_subjects/final_persistant_phenotype_dataset_test6.phen", row.names=FALSE,sep="\t",na="NA")
