...
#VIP_data_all$ffq<-0
#VIP_data_all$ffq[enkver2=="long"]<-1
#VIP_data_all$ffq[enkver2=="apri"]<-1
#VIP_data_all$ffq_factor<-as.factor(VIP_data_all$ffq)
#
##age squared
#VIP_data_all$agesq<-age*age
#
##factorize gender
#VIP_data_all$gender_factor<-as.factor(VIP_data_all$gender)
#
##adjust alko for TEI
#attach(VIP_data_all)
#alkosum1_mg<-round(1000*alkosum1)
##keep missing values first
#alkosum1_mg_residuals<-alkosum1_mg+ensum1
#model_znb<-zeroinfl(alkosum1_mg~ensum1, dist="negbin")
#summary(model_znb)

#get residuals
#alkosum1_mg_residuals[!is.na(VIP_data_all$alkosum1) & !is.na(VIP_data_all$ensum1)]<-model_znb$residuals
