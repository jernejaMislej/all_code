...
VIP_data_independant_complete_cases$environment_score<-apply(as.matrix(apply(cbind(VIP_data_independant_complete_cases[,48],VIP_data_independant_complete_cases[,11]-1),2,scale))%*%
				diag(multiple_coefficients),1,sum,na.rm=T)

VIP_data_subset_visit1_complete_cases$environment_score<-apply(as.matrix(apply(cbind(VIP_data_subset_visit1_complete_cases[,49],VIP_data_subset_visit1_complete_cases[,11]-1),2,scale))%*%
				diag(multiple_coefficients),1,sum,na.rm=T)

VIP_data_subset_visit2_complete_cases$environment_score<-apply(as.matrix(apply(cbind(VIP_data_subset_visit2_complete_cases[,49],VIP_data_subset_visit2_complete_cases[,11]-1),2,scale))%*%
				diag(multiple_coefficients),1,sum,na.rm=T)
